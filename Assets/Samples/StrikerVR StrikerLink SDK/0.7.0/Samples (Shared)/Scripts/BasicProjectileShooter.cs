...
namespace StrikerLink.Unity.Runtime.Samples
{
    public class BasicProjectileShooter : MonoBehaviour
    {
        ...
        void CheckInputs()
        {
            if (strikerDevice.GetTriggerDown() || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButtonDown(0)))
            {
                if (currentAmmo > 0)
                    Fire();
                else
            ...
            else if (strikerDevice.GetSensorDown(Shared.Devices.DeviceFeatures.DeviceSensor.ReloadTouched) || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButtonDown(1)))
            {
                if(currentAmmo < ammo)
                    Reload();
            }
