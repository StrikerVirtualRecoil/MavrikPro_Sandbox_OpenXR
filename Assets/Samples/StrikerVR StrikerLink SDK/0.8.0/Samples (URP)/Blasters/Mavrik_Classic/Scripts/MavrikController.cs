...
namespace StrikerLink.Unity.Runtime.Samples
{
    public class MavrikController : MonoBehaviour
    {
        ...
        void CheckInputs()
        {
            if (!autoMode && strikerDevice.GetTriggerDown() || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButtonDown(0)))
            {
                if (currentAmmo > 0)
                    Fire();
                else
            ...
            if (autoMode && strikerDevice.GetTrigger() || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButton(0)))
            {
                if (currentAmmo > 0)
                    Fire();
            }

            if (strikerDevice.GetSensorDown(Shared.Devices.DeviceFeatures.DeviceSensor.ReloadTouched) || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButtonDown(1)))
            {
                if (currentAmmo < ammoSize)
                    Reload();
            }

            if (strikerDevice.GetButtonDown(Shared.Devices.DeviceFeatures.DeviceButton.SideLeft) || strikerDevice.GetButtonDown(Shared.Devices.DeviceFeatures.DeviceButton.SideRight) || (Application.isEditor && mouseDebugInput && UnityEngine.Input.GetMouseButtonDown(2)))
            {
                autoMode = !autoMode;
            }
